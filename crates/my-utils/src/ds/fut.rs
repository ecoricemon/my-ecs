...
impl FutureHelper {
    ...
    /// # Note
    ///
    /// This method could busy-wait. But the future is not desinged to be accessed from multiple
    /// thread at the same time. If the future should work in such circumstances, it would be
    /// better to use lock mechanism.
    fn lock_free(
        ptr_state: NonNull<AtomicU16>,
        clear_mask: u16,
        set_flags: u16,

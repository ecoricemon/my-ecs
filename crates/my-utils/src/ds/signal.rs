...
impl Signal {
    ...
    pub fn set_handles(&mut self, handles: Vec<Thread>) -> Vec<Thread> {
        let init_state = if self.states.is_empty() {
            ...
            .states
            .iter()
            .all(|state| state.load(Ordering::SeqCst) == ASLEEP)
        {
            ASLEEP
        } else {
        ...
        // Replaces 'states'.
        self.states = (0..new_len).map(|_| AtomicU8::new(init_state)).collect();
    ...
    pub fn wait(&self, this_index: usize) {
        #[cfg(debug_assertions)]
        {
            ...
            assert_eq!(thread::current().id(), handle.id(), "incorrect index found");
        }
            ...
            .is_ok()
        {
            loop {
                ...
                if self.states[this_index].load(Ordering::SeqCst) != ASLEEP {
                    break;
                }
            }
        }
        ...
        // ALARM -> AWAKE.
        self.states[this_index].store(AWAKE, Ordering::SeqCst);
    }
    ...
    pub fn notify(&self, target_index: usize) {
        ...
        for i in (0..len).cycle().skip(target_index).take(len * RETRY) {
            if self.states[i]
                .compare_exchange(ASLEEP, ALARM, Ordering::SeqCst, Ordering::SeqCst)
                .is_ok()
            {
        ...
        self.states[target_index].store(ALARM, Ordering::SeqCst);
    ...
    pub fn notify_all(&self) {
        for (state, handle) in self.states.iter().zip(self.handles.iter()) {
            state.store(ALARM, Ordering::SeqCst);

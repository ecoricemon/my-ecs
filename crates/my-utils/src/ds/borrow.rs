...
impl<V: Clone> SimpleHolder<V> {
    /// Creates a new holder that lends cloned values.
    ...
    /// Consumes the holder and returns the inner value.
...
/// A type generated by [`Holder`] that wraps a value shallowly.
///
/// This struct implements [`Deref`] and [`DerefMut`] for the inner value so that clients can use
/// this struct like the inner value.
///
/// If the `check` feature is enabled, this struct tracks borrow status at runtime and warns you if
/// the borrow rules are violated.
#[derive(Debug)]
...
impl<B> Borrowed<B> {
    ...
    /// Clones the borrowed value while preserving borrow tracking.
...
pub enum BorrowError {
    ...
    /// Other borrow error with a message.
    Other(String),
}

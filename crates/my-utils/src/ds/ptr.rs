...
impl<T: ?Sized> ManagedConstPtr<T> {
    ...
    fn _new(
        inner: NonNullExt<T>,
        #[cfg(feature = "check")] is_track: bool,
        #[cfg(feature = "check")] source: NonNullExt<T>,
    ) -> Self {
        #[cfg(feature = "check")]
        if is_track {
            debug::insert_const_ptr(source);
        }
    ...
    pub fn cast<U>(self) -> ManagedConstPtr<U> {
        ...
        #[cfg(feature = "check")]
        {
            ...
            let source = self.source.cast();

            drop(self);

            ManagedConstPtr::_new(inner, is_track, source)
        }
    }
        ...
        T: Sized,
    {
        ...
        #[cfg(feature = "check")]
        {
            // In check mode, we keep the source address even though arithmetic on the pointer takes
            // place. It's something like provenance of the memory.
...
impl<T: ?Sized> ManagedMutPtr<T> {
    ...
    pub fn cast<U>(self) -> ManagedMutPtr<U> {
        ...
        #[cfg(feature = "check")]
        {
            ...
            ManagedMutPtr::_new(inner, is_track, source)
        }
    }
...
mod debug {
    ...
    pub(super) fn insert_mut_ptr<T: ?Sized>(ptr: NonNullExt<T>) {
        // If `T` is a zero-sized type, then it's good to be allowed to have the same pointers
        // because they do not mutate the same data. It can be considered as a const pointer.

    ...
    S: BuildHasher + Default + Clone + 'static,
{
    ...
    unsafe fn get_column(&self, ci: usize) -> Option<NonNull<u8>> {
        self.cols.get(ci).map(|col| col.vec_ptr().cast())
    }
}
...
impl Column {
    ...
    #[inline]
    unsafe fn vec_ptr(&self) -> NonNull<AnyVec> {
        self.holder.ptr_inner()
    }
}

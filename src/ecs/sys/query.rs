...
use crate::{ds::prelude::*, ecs::resource::ResourceKey, util::Take};
...
impl<'buf, R: Query> Take for Read<'buf, R>
where
    R::Output<'buf>: Take,
{
    type Inner = <R::Output<'buf> as Take>::Inner;
...
impl<'buf, W: QueryMut> Take for Write<'buf, W>
where
    W::Output<'buf>: Take,
{
    type Inner = <W::Output<'buf> as Take>::Inner;
...
impl<'buf, RR: ResQuery> Take for ResRead<'buf, RR>
where
    RR::Output<'buf>: Take,
{
    type Inner = <RR::Output<'buf> as Take>::Inner;
...
impl<'buf, RW: ResQueryMut> Take for ResWrite<'buf, RW>
where
    RW::Output<'buf>: Take,
{
    type Inner = <RW::Output<'buf> as Take>::Inner;
...
impl<'buf, EW: EntQueryMut> Take for EntWrite<'buf, EW>
where
    EW::Output<'buf>: Take,
{
    type Inner = <EW::Output<'buf> as Take>::Inner;
